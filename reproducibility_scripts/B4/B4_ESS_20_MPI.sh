...
INPUT_PATH=inputs/reproducibility/TEMPLATE_B4_eSSm.xml
OUTPUT_PATH=output/B4_ESS_20_MPI
NUM_PROC=20 # NUMBRE OF MPI PARALLEL PROCESSORS
...
for i in `seq $INIT_RUN $NUMBER_OF_RUNS`; do
   echo RUN$i
   if [ "$i" -lt "10" ]; then
	/home/david.penas/openmpi/bin/mpirun -np $NUM_PROC bin/paralleltestbed $INPUT_PATH "$OUTPUT_PATH"_0$i
   else
        /home/david.penas/openmpi/bin/mpirun -np $NUM_PROC bin/paralleltestbed $INPUT_PATH "$OUTPUT_PATH"_$i
   fi 
done
cd $CURRENT_PWD
##################################################################################################################
